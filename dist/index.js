...
class ZKCompressionDocsServer {
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            const { query, limit = 5, section } = request.params.arguments;
            if (!query) {
    ...
    async searchDocs(query, limit, section) {
        if (!this.searchIndex) {
            return {
                content: [
                    {
        ...
        if (section) {
            searchResults = searchResults.filter(result => result.item.section.toLowerCase().includes(section.toLowerCase()));
        }
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            for (const line of contentLines) {
                if (line.trim() === '') {
                    if (currentParagraph.trim() && queryWords.some(word => currentParagraph.toLowerCase().includes(word))) {
