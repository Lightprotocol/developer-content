...
<Steps>
<Step title="millions of accounts are compressed into a 'fingerprint' - the Merkle tree root hash"/>
<Step title="this 'fingerprint' is stored in one Solana account"/>
<Step title="the account history is stored on the Solana ledger"/>
<Step title="the latest compressed data is fetched from an indexer"/>
<Step title="to verify the data, recompute the hashes and compare the final hash to the on chain root hash."/>
</Steps>
