...
<table><thead><tr><th width="150">Type</th><th width="178">Lamports</th><th>Notes</th></tr></thead><tbody><tr><td>Solana base fee</td><td>5000 per signature</td><td>~</td></tr><tr><td>Write new compressed account state </td><td>~100-300 per leaf</td><td>Depends on tree depth: <br><span class="math">\left( 2^{\text{tree\_depth}} \times \text{tree\_account\_rent\_cost} \times \text{rollover\_threshold} \right) </span></td></tr><tr><td>Nullify old compressed account state</td><td>5000</td><td>Currently, this flat fee applies when nullifying at least one account to reimburse the cost of running a forester transaction. The current default Forester node implementation can be found <a href="../../node-operators/run-a-node.md#light-forester-node">here</a>.</td></tr><tr><td>Create an address</td><td>5000</td><td>Same as Nullify.</td></tr></tbody></table>
